 ...
 *
 * <p>Setup: one scratch Postgres, shared with the other SQL classes of this suite.
 * A minimal {@code orchestrator.workflow_runs} schema is rebuilt (only the
 ...
 *
 * <p><b>How it runs.</b> It used to start its own container and set
 * {@code disabledWithoutDocker = true}, which meant it SKIPPED wherever Docker was absent -
 * and the {@code arc-build} CI runners expose no Docker socket, so that was everywhere that
 * matters. It was also named {@code ...IT} and selected by no {@code -Dtest} list, so it ran
 * nowhere at all. For the one SQL path {@code AGENTS.md} marks CRITICAL - every write to
 * {@code workflow_runs.state_snapshot} goes through {@link JsonbPatchExecutor}, and a rogue
 * write diverges silently and surfaces hours later - a proof that skips is worse than no
 * proof, because it reports green.
 *
 * <p>It now takes the Postgres its CI job already runs, through {@link ScratchPostgres}:
 * skipped on a laptop with no scratch database, a hard FAILURE on CI, and a refusal for any
 * URL that does not visibly name a scratch database. The unit-test suite still validates the
 * same builders without a database.
 */
class JsonbPatchPostgresTest {
    ...
    private static final ScratchPostgres DB = ScratchPostgres.forPrefix(
            ...
            "it is the only executable proof that the SQL JsonbPatchExecutor composes for each "
                    + "of the 13 wired patch types actually runs on Postgres, on the one write "
                    + "path AGENTS.md marks CRITICAL");
    ...
    static void setUpClass() {
        ...
        //
        // The table is REBUILT rather than reused. The scratch database is shared with the
        // other converted classes, and CasContentionPostgresTest declares this same table with
        // an incompatible shape (id UUID, no tenant_id, plus state_snapshot_seq). Whichever
        // ran first would decide the columns for the other, so "CREATE TABLE IF NOT EXISTS"
        // here would silently test against someone else's schema.
        jdbc.getJdbcTemplate().execute("CREATE SCHEMA IF NOT EXISTS orchestrator");
        jdbc.getJdbcTemplate().execute("DROP TABLE IF EXISTS orchestrator.workflow_runs CASCADE");

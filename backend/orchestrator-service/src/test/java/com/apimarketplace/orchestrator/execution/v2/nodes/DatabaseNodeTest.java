...
class DatabaseNodeTest {
    ...
    void templatedPortAndTimeoutAreResolved() {
        Core.DatabaseConfig config = new Core.DatabaseConfig("postgresql", null, null, "mydb", "user", "pass", null, "SELECT 1", null, "select", null, null);
        DatabaseNode node = new DatabaseNode("core:database", config);
        node.setDeferredScalars(java.util.Map.of("database", java.util.Map.of(
        ...
        org.mockito.Mockito.lenient().when(adapter.resolveTemplates(
            ...
            .thenAnswer(TemplateResolutionStubs.resolving(java.util.Map.of(
                "{{core:x.output.port}}", "6543", "{{core:x.output.timeout}}", 45000)));
        ...
        assertEquals(6543, params.get("port"));

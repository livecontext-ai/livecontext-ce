...
public class ApplicationHelpModule implements ToolModule {
    ...
    private Map<String, Object> buildResponseFieldDocs() {
        ...
        r.put("node_types", "array of strings - the node types the app is built from ('mcp:gmail', 'core:loop', 'agent:agent', 'trigger:webhook', 'table:find', 'interface'). Present on 'my' items, absent when the app reports none. These are the exact tokens the `node_types` parameter accepts, so read them here instead of guessing how an integration is spelled.");
    ...
    private Map<String, Object> buildParameterDocs() {
        ...
        params.put("node_types", "array of string - Keep only applications containing one of these node types (my). "
                + "ANY-of, applied before pagination. Tokens look like 'mcp:gmail', 'core:loop', 'agent:agent', "
                + "'trigger:webhook', 'table:find', 'interface'. Each item in the 'my' response reports its own node_types.");

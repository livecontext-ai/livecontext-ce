 ...
 *
 * <p>The output drives the node-type filter on the workflow and application
 * lists. In this service it is computed on demand from the plan the list has
 * already loaded ({@code WorkflowEntity.getNodeTypes()}); the publication twin
 * stores its result in the {@code node_types} column, because its list queries
 * deliberately never load a plan snapshot. Neither is indexed - both surfaces
 * are scope-bounded and filter in memory.
 *
 ...
 *
 * <p>Twin: {@code publication-service} {@code WorkflowNodeTypeExtractor} writes
 * the same tokens onto published rows. Both must stay in sync - pinned by
 * {@code WorkflowNodeTypeExtractorParityTest}.
 */

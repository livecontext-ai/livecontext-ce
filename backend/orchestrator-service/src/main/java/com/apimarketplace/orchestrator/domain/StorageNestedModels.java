...
/**
 * Domain models for nested JSON navigation in Storage
 * Supports hierarchical navigation through JSON structures stored in storage
 */
public final class StorageNestedModels {
    private StorageNestedModels() {}

    /**
     * Nested Storage row with JSON path information
     */
    public record StorageNestedRow(
            Long id, // Index in the array or ID
            @JsonProperty("storage_id") UUID storageId,
            @JsonProperty("tenant_id") String tenantId,
            @JsonProperty("json_path") String jsonPath, // Full JSON path (e.g., "user.address")
            @JsonProperty("parent_path") String parentPath, // Parent path (e.g., "user")
            Map<String, Object> data, // Extracted data at the path
            @JsonProperty("updated_at") Instant updatedAt
    ) {
        public StorageNestedRow {
            Objects.requireNonNull(id, "id cannot be null");
            Objects.requireNonNull(storageId, "storageId cannot be null");
     ...
     */
    public record StorageColumnDefinition(
            ...
            @JsonProperty("frequency") Integer frequency, // Number of items having this column (optional)
            @JsonProperty("total_items") Integer totalItems // Total items analyzed (optional)
    ) {
    ...
    public record NestedDataRequest(
            @JsonProperty("json_path") String jsonPath, // JSON path to extract (e.g., "user.address")

...
public class WorkflowBuilderToolDefinitionFactory {
    ...
    public AgentToolDefinition buildToolDefinition() {
        List<ToolParameter> params = List.of(
            stringParam("action", "Action: init, load, save, finish, add_node, run_node, connect, disconnect, modify, remove, undo, describe, validate, search, execute, present, stop_run, restart_from_node, get, list, delete, runs, get_run, wait_run, get_node_output, get_plan, set_plan, pin, unpin, publish, unpublish, resolve_approval, continue_interface, mock_suggest, help. " +
            ...
            stringParam("view", "(for: present, not 'show' which means describe) What the user should look at now, opened in their side panel: 'application' (a run's interfaces, run_id), 'run' (a run's steps, run_id), 'workflow' (workflow_id). Changes nothing, only the user's view. A table, interface, agent or file is presented by its own tool (action='present').", false),
            stringParam("run_id", "Run ID (for: get_run, wait_run, stop_run, restart_from_node, get_node_output, present view='application' or 'run'). On stop_run, omit ONLY when you are an agent running inside a workflow and want to stop your own run.", false),

...
/**
 * Moves {@code orchestrator.approval_channel_deliveries.callback_token} (the token inside a
 * Telegram approval button) from plaintext to encrypted + hashed storage (see
 * {@code TokenAtRest}): once at startup, and row by row on a tap whose hash lookup missed.
 */
@Component
public class OrchestratorTokenAtRestBackfill {

    public static final TableSpec APPROVAL_CALLBACK_TOKENS =
            new TableSpec("orchestrator.approval_channel_deliveries", "id", "callback_token", "callback_token_hash");

    static final List<TableSpec> TABLES = List.of(APPROVAL_CALLBACK_TOKENS);
     ...
     */
    public OrchestratorTokenAtRestBackfill(JdbcTemplate jdbc,

package com.apimarketplace.datasource.config;

import org.springframework.boot.context.properties.ConfigurationProperties;
import org.springframework.context.annotation.Configuration;

/**
 * Thin datasource-service-local view over the {@code ai.agent.defaults.*} YAML keys
 * the table tool module needs. Exposes the uniform per-resource cap so ops can
 * override it without a rebuild.
 *
 * <p>This is intentionally separate from agent-service's {@code AgentDefaultsConfig} -
 * datasource-service does not depend on agent-service at compile time, and binding
...
public class DataSourceAgentDefaultsConfig {

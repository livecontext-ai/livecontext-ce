package com.apimarketplace.datasource.archunit;
...
/**
 * datasource-service mirror of orchestrator's
 ...
 * design rationale.
 */
@DisplayName("ScopeGuard callsite invariants (datasource-service mirror)")
class OrgScopePredicateInvariantTest {

    private static final List<String> RULE_1_ALLOWLIST = List.of(
            // DataSourceCrudController routes through ScopeGuard.isInStrictScope.
            // Internal services that read both getters do so for stamping/persistence
            // (event publishing, row snapshot), not as a scope predicate.
    );

    private final JavaClasses classes = new ClassFileImporter()
            .withImportOption(ImportOption.Predefined.DO_NOT_INCLUDE_TESTS)
            .withImportOption(ImportOption.Predefined.DO_NOT_INCLUDE_JARS)
            .importPackages("com.apimarketplace.datasource");
    ...
    void rule1NoHandRolledScopePredicate() {
        ...
        assertThat(offenders)
                .as("datasource-service: hand-rolled scope predicate detected.")
                .isEmpty();
    }
    ...
    void rule2TolerantCallSitesMustBeAnnotated() {
        ...
        assertThat(offenders)
                .as("datasource-service: isInOwnerOrOrgScope without @TolerantScope.")
                .isEmpty();
    }

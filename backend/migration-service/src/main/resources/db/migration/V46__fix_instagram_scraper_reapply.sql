-- ============================================================================
-- V46: Re-apply Instagram Scraper API + tool_responses
-- V43/V44 ran as no-ops on DBs where categories were missing at migration time.
-- This migration is self-contained and idempotent (ON CONFLICT DO NOTHING).
-- ============================================================================
...
BEGIN
    -- 0a. Ensure category "Social Media" exists
    ...
    -- 0b. Ensure subcategory "Instagram" exists
    ...
    RAISE NOTICE 'V46: Instagram Scraper API re-applied with 13 endpoints';
END $$;

-- Also ensure tool_responses are active for this API
UPDATE catalog.tool_responses SET is_active = true
WHERE tool_id IN (SELECT id FROM catalog.api_tools WHERE api_id = '9b962417-1510-450a-b6a6-6b7649146cff');

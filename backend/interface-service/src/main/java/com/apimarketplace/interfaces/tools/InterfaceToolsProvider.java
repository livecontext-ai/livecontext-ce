    ...
    private AgentToolDefinition buildUnifiedInterfaceTool() {
        List<ToolParameter> params = List.of(
            ...
            stringParam("format", "Shape this interface is designed for (for: create, update). Drives the dimensions "
                + "of every screenshot, video and preview of it. Presets: classic (1280x800) | widescreen (1920x1080) | "
                + "vertical (1080x1920) | square (1080x1080) | portrait (1080x1350) | mobile (390x844) | "
                + "tablet (820x1180) | desktop (1440x900) | banner (1500x500) | social_card (1200x630) | "
                + "a4_portrait (794x1123) | a4_landscape (1123x794); aliases landscape, horizontal, story, reel, og, "
                + "16:9, 9:16, 1:1, 4:5; or a custom \"WIDTHxHEIGHT\" with each side 16-2160. Match html_template's "
                + "viewport meta + layout to the format's width. OMIT for a tall page you want captured whole: with no "
                + "format the screenshot is a full-page capture at 1280 wide (format='classic' instead crops to an "
                + "exact 1280x800 frame). On update, format='' clears it back to no declared shape. An invalid value "
                + "is rejected with the preset list.", false),
            stringParam("html_template", "HTML content - REQUIRED for create. Only {{name|default}} placeholders are resolved - any other template syntax renders as raw text; put logic in js_template. Names are generic (e.g. {{title|Untitled}}, {{items|No data}}). In a workflow, the interface node's variable_mapping maps these to real data (e.g. variable_mapping:{title:'{{agent:x.output.response}}'}). MUST include a fixed-width viewport meta matching this interface's own `format`: <meta name=\"viewport\" content=\"width=1280\"> when you set no format, otherwise width=<the format's width> (e.g. width=1080 for 'vertical'/'square'/'portrait', width=1920 for 'widescreen'). Base styles: a FRAGMENT (no <!DOCTYPE>/<html>) inherits the platform base (system font, border-box, 8px body padding), overridable from css_template; a COMPLETE document starting with <!DOCTYPE html> or <html> inherits NOTHING and renders exactly as authored, identically in preview, screenshot and video - reset body margin yourself there.", false),

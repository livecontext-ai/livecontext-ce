...
/**
 * interface-service mirror of orchestrator's {@code OrgScopePredicateInvariantTest}.
 * Same rule logic, different classpath scan. See the reference implementation for
 * the contract + design rationale. InterfaceService carries 10+ hand-written
 * ScopeGuard call sites; this mirror keeps future predicates funneled through it.
 */
@DisplayName("ScopeGuard callsite invariants (interface-service mirror)")
class OrgScopePredicateInvariantTest {

    private static final List<String> RULE_1_ALLOWLIST = List.of(
            // DTO serialization: reads both getters to project the row, no predicate.
            "InterfaceDtoMapper#toDto",
            // Scope is routed through findInScope (ScopeGuard inside); the getter reads
            // are the canWrite argument + owner-tenant storage accounting / strict
            // delete routing, not a hand-rolled predicate.
            "InterfaceService#updateInterface",
            "InterfaceService#deleteInterface"
    );

    private final JavaClasses classes = new ClassFileImporter()
            .withImportOption(ImportOption.Predefined.DO_NOT_INCLUDE_TESTS)
            .withImportOption(ImportOption.Predefined.DO_NOT_INCLUDE_JARS)
            .importPackages("com.apimarketplace.interfaces");
    ...
    void rule1NoHandRolledScopePredicate() {
        ...
        assertThat(offenders)
                .as("interface-service: hand-rolled scope predicate detected.")
                .isEmpty();
    }
    ...
    void rule2TolerantCallSitesMustBeAnnotated() {
        ...
        assertThat(offenders)
                .as("interface-service: isInOwnerOrOrgScope without @TolerantScope.")
                .isEmpty();
    }

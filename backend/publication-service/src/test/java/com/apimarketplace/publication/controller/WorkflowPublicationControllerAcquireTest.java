...
@DisplayName("WorkflowPublicationController acquire validation surfacing")
class WorkflowPublicationControllerAcquireTest {
    ...
    @Test
    @DisplayName("Acquire surfaces a service validation IllegalArgumentException as HTTP 400 with the reason (not 500)")
    void acquireValidationErrorSurfacesAsBadRequest() {
        UUID publicationId = UUID.fromString("11111111-1111-1111-1111-111111111111");
        String reason = "Publication is not active";
        when(publicationService.acquirePublication(publicationId, "buyer-1", "org-1"))
                .thenThrow(new IllegalArgumentException(reason));

        ResponseEntity<?> response = controller.acquirePublication(
                "buyer-1", "org-1", "MEMBER", publicationId.toString());

        // Validation failures are client errors: the message must reach the caller as 400, not be
        // masked by the generic 500 catch-all.

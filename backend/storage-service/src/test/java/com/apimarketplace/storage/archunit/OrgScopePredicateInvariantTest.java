package com.apimarketplace.storage.archunit;
...
/**
 * storage-service mirror of orchestrator's {@code OrgScopePredicateInvariantTest}.
 * Same rule logic, different classpath scan (storage exposes controllers under
 * {@code .web}). See the reference implementation for the contract + rationale.
 */
@DisplayName("ScopeGuard callsite invariants (storage-service mirror)")
class OrgScopePredicateInvariantTest {
    ...
    private final JavaClasses classes = new ClassFileImporter()
            ...
            .importPackages("com.apimarketplace.storage");
    ...
    void rule1NoHandRolledScopePredicate() {
        ...
        assertThat(offenders)
                .as("storage-service: hand-rolled scope predicate detected.")
                .isEmpty();
    }
    ...
    void rule2TolerantCallSitesMustBeAnnotated() {
        ...
        assertThat(offenders)
                .as("storage-service: isInOwnerOrOrgScope without @TolerantScope.")
                .isEmpty();
    }

    private boolean isCandidateClass(JavaClass c) {
        String pkg = c.getPackageName();
        return pkg.contains(".web")

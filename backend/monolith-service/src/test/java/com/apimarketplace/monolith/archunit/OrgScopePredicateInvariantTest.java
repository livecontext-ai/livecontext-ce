package com.apimarketplace.monolith.archunit;
...
/**
 * monolith-service (CE) mirror of orchestrator's {@code OrgScopePredicateInvariantTest}.
 * The monolith re-hosts the other services' beans but ALSO ships its own glue
 * (MonolithFileController, MonolithChannelAuthorizer, WS handler) that the other
 * mirrors never scan - without this test that CE-only glue could grow a
 * hand-rolled scope predicate unnoticed.
 */
@DisplayName("ScopeGuard callsite invariants (monolith-service CE mirror)")
class OrgScopePredicateInvariantTest {

    private static final List<String> RULE_1_ALLOWLIST = List.of(
            // Bean-factory wiring: maps a SharedLink row into ShareTokenContext (DTO
            // projection of owner tenant + org for the security filter). No predicate.
            "MonolithSecurityConfig#monolithSecurityFilter"
    );

    private final JavaClasses classes = new ClassFileImporter()
            .withImportOption(ImportOption.Predefined.DO_NOT_INCLUDE_TESTS)
            .withImportOption(ImportOption.Predefined.DO_NOT_INCLUDE_JARS)
            .importPackages("com.apimarketplace.monolith");
    ...
    void rule1NoHandRolledScopePredicate() {
        ...
        assertThat(offenders)
                .as("monolith-service: hand-rolled scope predicate detected.")
                .isEmpty();
    }
    ...
    void rule2TolerantCallSitesMustBeAnnotated() {
        ...
        assertThat(offenders)
                .as("monolith-service: isInOwnerOrOrgScope without @TolerantScope.")
                .isEmpty();
    }

    private boolean isCandidateClass(JavaClass c) {
        // The monolith module is small CE glue; scan everything except config.
        return !c.getPackageName().contains(".config");
    }

package com.apimarketplace.conversation.archunit;
...
/**
 * conversation-service mirror of orchestrator's
 ...
 * design rationale.
 */
@DisplayName("ScopeGuard callsite invariants (conversation-service mirror)")
class OrgScopePredicateInvariantTest {

    private static final List<String> RULE_1_ALLOWLIST = List.of(
            // Stamping / persistence - copies userId + organizationId from the
            // request DTO onto a new Conversation entity, not a scope predicate.
            "ConversationCommandService#createConversation",
            "ConversationCommandService#persistConversation",
            // MessageService.addMessage reads conversation.getUserId() (for
            // breakdown tracking) and conversation.getOrganizationId() (to
            // forward into compactionOrchestrator.afterTurnAsync). Both are
            // routing decisions, not gates - the conversation was already
            // scope-checked upstream by isConversationInScope before reaching
            // addMessage. Pure value-forwarding into a downstream call.
            "MessageService#addMessage"
            // MessageService.isConversationInScope routes through ScopeGuard.
            // ConversationCommandService.deleteConversationsByWorkflowId/byAgentId
            // route through ScopeGuard.isInStrictScope per-row.
            // ConversationSharingService.assertAuthorized routes through
            // ScopeGuard.isInOwnerOrOrgScope with @TolerantScope.
    );

    private final JavaClasses classes = new ClassFileImporter()
            .withImportOption(ImportOption.Predefined.DO_NOT_INCLUDE_TESTS)
            .withImportOption(ImportOption.Predefined.DO_NOT_INCLUDE_JARS)
            .importPackages("com.apimarketplace.conversation");
    ...
    void rule1NoHandRolledScopePredicate() {
        ...
        assertThat(offenders)
                .as("conversation-service: hand-rolled scope predicate detected.")
                .isEmpty();
    }
    ...
    void rule2TolerantCallSitesMustBeAnnotated() {
        ...
        assertThat(offenders)
                .as("conversation-service: isInOwnerOrOrgScope without @TolerantScope.")
                .isEmpty();
    }

...
/**
 * CE-side bookkeeping of the last bundle fetch/apply. Single row (id=1) seeded
 * by V114. See also {@link CatalogBundleEntity} which persists the actual
 * signed payload; this row only tracks the FETCH side (which can fail without
 * producing a bundle row).
 */
@Entity
@Table(name = "catalog_bundle_sync_status")
public class CatalogBundleSyncStatusEntity {

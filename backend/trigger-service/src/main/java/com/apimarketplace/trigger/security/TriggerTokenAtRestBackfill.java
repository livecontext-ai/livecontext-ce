...
/**
 * Moves the four trigger token tables from plaintext to encrypted + hashed storage (see
 * {@code TokenAtRest}): once at startup, and row by row on a lookup that missed on the hash,
 * plus a read-only plaintext fallback for the lookups that arrive before that pass.
 */
@Component
public class TriggerTokenAtRestBackfill {

    public static final TableSpec STANDALONE_WEBHOOKS =
            new TableSpec("trigger.standalone_webhooks", "id", "token", "token_hash");
    public static final TableSpec STANDALONE_CHAT_ENDPOINTS =
            new TableSpec("trigger.standalone_chat_endpoints", "id", "token", "token_hash");
    public static final TableSpec STANDALONE_FORM_ENDPOINTS =
            new TableSpec("trigger.standalone_form_endpoints", "id", "token", "token_hash");
    public static final TableSpec WEBHOOK_TOKENS =
            new TableSpec("trigger.webhook_tokens", "id", "token", "token_hash");

    static final List<TableSpec> TABLES = List.of(
            STANDALONE_WEBHOOKS, STANDALONE_CHAT_ENDPOINTS, STANDALONE_FORM_ENDPOINTS, WEBHOOK_TOKENS);
     ...
     */
    public TriggerTokenAtRestBackfill(JdbcTemplate jdbc,

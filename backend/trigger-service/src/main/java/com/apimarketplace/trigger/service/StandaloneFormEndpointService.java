...
public class StandaloneFormEndpointService {
    ...
    public Optional<StandaloneFormEndpointDto> findByToken(String token) {
        return TokenAtRest.lookup(token, formEndpointRepository::findByTokenHash,
                t -> tokenBackfill == null ? Optional.empty() : tokenBackfill.findLegacy(TriggerTokenAtRestBackfill.STANDALONE_FORM_ENDPOINTS, t, formEndpointRepository::findLegacyPlaintext)).map(this::toDto);
    }

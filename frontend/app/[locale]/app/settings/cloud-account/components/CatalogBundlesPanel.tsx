...
export default function CatalogBundlesPanel() {
  ...
  const handleDelete = async (id: number) => {
    ...
    setError(null);
    try {
      await modelConfigService.deleteBundle(id);
      await refresh();
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : String(e);
      setError(msg);
                        ...
                        {!b.isActive && (
                          <span className="inline-flex items-center gap-1.5">
                            <Button
                              ...
                              disabled={activatingId === b.id || deletingId === b.id}
                            ...
                            <Button
                              ...
                              disabled={deletingId === b.id || activatingId === b.id}
                              aria-label={tb("deleteBundle")}
                              title={tb("deleteBundle")}
                              className="h-7 px-2 text-xs text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20"
                            >
                              {deletingId === b.id ? (
                                <Loader2 className="w-3 h-3 animate-spin" />
                              ) : (

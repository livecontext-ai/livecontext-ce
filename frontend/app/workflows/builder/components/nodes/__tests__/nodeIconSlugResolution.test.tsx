 ...
 *     Because that request succeeds, `onError` never fired and the integration
 *     icon fallback was unreachable too;
...
vi.mock('@/components/ThemeProvider', () => ({
  ...
  useOptionalTheme: () => ({ theme: themeState.theme }),
}));
...
afterEach(() => { themeState.theme = 'light'; });
...
describe('NodeIcon - static icon slug resolution', () => {
  ...
  it('Regression - the catalog "mcp" sentinel never renders as a service icon', () => {
    ...
    // Falls through to the integration icon instead of the generic /icons/services/mcp.svg.
    expect(iconSrc()).not.toContain('/icons/services/');
    expect(iconSrc()).toContain('/icons/integration_black.svg');
  });
  ...
  it('Gives up to the integration icon when both the verbatim and normalized slugs 404', () => {
  ...
  it('A slug with no normalized variant gives up after a single 404', () => {
    ...
    // make - one error must land straight on the integration icon rather than
  ...
  it('An integration node with no slug at all falls back to the integration icon', () => {
    render(<NodeIcon isMcp nodeId="mcp-unknown" />);
    expect(iconSrc()).toContain('/icons/integration_black.svg');
    expect(screen.getByTestId('node-icon-img')).toHaveAttribute('alt', 'Integration');
  });

  it('Dark theme uses the light-stroke integration icon', () => {
    themeState.theme = 'dark';
    render(<NodeIcon isMcp nodeId="mcp-unknown" />);
    expect(iconSrc()).toBe('/icons/integration.svg');
  });

  // MCP Interface / MCP Resource really are the Model Context Protocol, so they
  // must keep the MCP logo instead of the generic integration icon.
  it.each([
    [{ nodeId: 'mcp-interface' }],
    [{ nodeId: 'mcp-resource' }],
    [{ nodeId: 'node-42', nodeFamily: 'mcp-resource' }],
  ])('A protocol MCP node %o keeps the MCP logo', (props) => {
    render(<NodeIcon isMcp {...props} />);
    expect(iconSrc()).toBe('/icons/mcp_black.png');
    expect(screen.getByTestId('node-icon-img')).toHaveAttribute('alt', 'MCP');
  });

  it('A protocol MCP node in dark theme uses the white MCP logo', () => {
    themeState.theme = 'dark';
    render(<NodeIcon isMcp nodeId="mcp-interface" />);
    expect(iconSrc()).toBe('/icons/mcp.png');
  });
});

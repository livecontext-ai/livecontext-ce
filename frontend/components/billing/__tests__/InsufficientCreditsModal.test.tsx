...
describe('InsufficientCreditsModal', () => {
  ...
  describe('what a credit buys, on the card the user is reading', () => {
    ...
    it('explains the Free grant with the plan card message, never the paid one', () => {
      ...
      // And NOT the paid sentence. It prices a turn on any model, while the FREE plan's
      // monthly credits pay for chat only on the models marked Free, so appending it
      // here would over-promise to the one reader who has just run out and is choosing
      // what to buy.
      expect(free).not.toContain(SHARED_TOOLTIP);
    });
  });

  describe('one Free pool', () => {
    const dialogText = () => document.body.textContent ?? '';

    it('regression: the Free column names no separate AI credits line, even if a plan row still carries one', () => {
      ...
      expect(text).toContain('features.freeCredits');
      expect(text).not.toContain('freeAiCredits');
      expect(text).not.toContain('aiCreditsFreeTooltip');
    });
  });
});

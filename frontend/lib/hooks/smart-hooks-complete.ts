...
export function useCreditBalance() {
  ...
  return {
    ...
    // react-query reports isLoading=false for a DISABLED query too (no session yet),
    // and every field above then reads as its own default, which is indistinguishable
    // from a real answer. A caller that only renders can ignore this; one that WRITES
    // something durable off the answer cannot.
    hasAnswered: data !== undefined,
    isLoading,
    error: error?.message || null,
  };
}
